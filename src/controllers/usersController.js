...
async function list(req, res) {
  try {
    ...
    const allowed = await hasPermissionWithoutRoleBypass(userId, "user.read");
    if (!allowed) return sendError(res, 403, "Permission denied: You don't have permission to list users", { code: CODES.FORBIDDEN, requestId: req.id });
...
async function getOne(req, res) {
  try {
    ...
    if (!userId) return sendError(res, 401, "Unauthorized", { code: CODES.UNAUTHORIZED, requestId: req.id });
    const allowed = await hasPermissionWithoutRoleBypass(userId, "user.read");
    if (!allowed) return sendError(res, 403, "Permission denied: You don't have permission to view this user", { code: CODES.FORBIDDEN, requestId: req.id });
